...
- Tests: Playwright (E2E), Vitest + React Testing Library (Unit/Component)
...
- Design tokens live in `client/styles/main.css` as CSS custom properties: `--color-primary: #d6b26e`, `--color-primary-light: #ead6ad`, `--color-primary-dark: #94691a`, `--color-surface: #faf7f2`, `--color-text: #3b342d`, `--color-text-light: #7a7268`, `--color-border: #ebe2d7`. Headings use Cormorant Garamond, body text uses Inter. Reuse these instead of introducing new colors or fonts.
...
- Paula's tagline also uses a dedicated colour, `--color-tagline: #996f13`, requested by Paula specifically for this element. Don't reuse it elsewhere or introduce further one-off colours without documenting them here first.
...
- All tests live under a single top-level `tests/` directory (sibling to `client/`), split by automation level: `tests/unit/`, `tests/component/`, and `tests/e2e/`. Create a subfolder the first time it's needed. Playwright (E2E) tests use the Page Object Model; the shared page object lives at `tests/e2e/pages/HomePage.ts`, since the site is single-page, add to that class instead of duplicating locators inline in new test files, run one file with `npx playwright test <path> --project=chromium`. Vitest + React Testing Library (Unit/Component) tests run in jsdom with `globals: true` and jest-dom matchers loaded via `client/test-setup.ts`; test files are named after what they cover (e.g. `Hero.test.tsx`) and placed in `tests/unit/` or `tests/component/` according to their level, run one file with `npx vitest run <path>` or the whole suite with `npm test`.
...
- Every feature's test cases are designed and classified by analyze-test-cases (/analyze-test-cases <feature-name>), which reads spec.md's acceptance criteria, flags ambiguities, classifies each case by ISTQB test type and automation level (Unit/Component/Integration/E2E) using the testing pyramid, and produces a confirmed test-plan.md (or delivers the plan directly in the reply, not saved to a file, unless asked to save it). Once confirmed, write-tests (/write-tests <feature-name>) takes that list (or any other already-classified list provided directly) and writes the actual test code: Playwright for E2E cases, following this project's Page Object Model (tests/e2e/pages/HomePage.ts), Vitest + React Testing Library for Unit/Component cases, then runs only that feature's file(s) on the chromium project (Playwright) or via vitest (Unit/Component). Touch/gesture interactions (taps, swipes) are left for manual QA across real devices instead of being automated, named explicitly rather than silently dropped; plain mobile-width layout checks (like horizontal overflow) still get automated. A failing test gets diagnosed as a test bug (write-tests fixes it, still just the test file), a resource-constrained crash (fixed once, never re-verified by rerunning the full file under the same memory pressure), or an app bug (never touched by this skill, reported for implement-task or a requirement discussion instead). Once implementation and tests are both settled, review-tests (/review-tests <feature-name>) runs right before closing the feature, a read-only audit that checks coverage and catches data drift (like a stale price) one final time. Address anything review-tests flags before final wrap-up. Neither verify-tasks nor close-feature ever run the Playwright suite themselves, this environment is memory-constrained; they rely on write-tests' last recorded result instead.
